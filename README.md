...
![Localize Extractor demo](demo/demo.gif)

> Upload a screenshot → OCR detects the words → click the text you want → localization keys and JSON are generated automatically.

The GIF above is generated by a scripted Playwright recording. To regenerate it (e.g. after UI changes):

```bash
cd frontend && npm run dev          # in one terminal
cd demo && npm install              # first time only
npx playwright install chromium     # first time only
npm run record                      # records + builds demo/demo.gif
```

If your dev server runs on a non-default port, pass it through: `APP_URL=http://localhost:5174 npm run record`. See [demo/](demo/) for the mock screenshot and scripts.
